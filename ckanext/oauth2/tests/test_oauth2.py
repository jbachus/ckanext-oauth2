...
class OAuth2PluginTest(unittest.TestCase):
    ...
    def test_get_token_with_no_credentials(self, oauth2_session_mock):
        ...
        oauth2.toolkit.request = make_request(
            True, "data.com", "callback", {"state": state}
        )
    ...
    def test_get_token_legacy_idm(self, OAuth2Session):
        ...
        expected_headers = {
            ...
            % urlsafe_b64encode("%s:%s" % (helper.client_id, helper.client_secret)),
        }
    ...
    def test_get_token_insecure(self):
        ...
        httpretty.register_uri(
            httpretty.POST, helper.token_endpoint, body=json.dumps(token)
        )
    ...
    def test_remember(self, headers):
        ...
        authenticator.remember.assert_called_once_with(
            environ, {"repoze.who.userid": user_name}
        )
    ...
    def test_challenge(self):
        ...
        # Check
        state = urlencode(
            {"state": b64encode(bytes(json.dumps({"came_from": came_from})))}
        )
        expected_url = (
    ...
    def test_identify(
        ...
        httpretty.register_uri(
            httpretty.GET, self._profile_api_url, body=json.dumps(user_info)
        )
        ...
        oauth2.model.User.by_email = MagicMock(
            return_value=[user] if user_exists else []
        )
    ...
    @httpretty.activate
    def test_identify_adds_new_user_to_default_org_with_role(self):
        helper = self._helper()
        oauth2.toolkit.config.update(
            {
                "ckan.oauth2.default_organization": "default-org",
                "ckan.oauth2.default_role": "editor",
            }
        )

        membership_action = MagicMock()
        oauth2.toolkit.get_action = MagicMock(return_value=membership_action)

        user_info = {
            self._user_field: "newuser",
            self._email_field: "new@example.com",
        }
        httpretty.register_uri(
            httpretty.GET,
        ...
        oauth2.toolkit.request = make_request(
            False, "localhost", "/oauth2/callback", {}
        )
        oauth2.model.Session = MagicMock()
        user = MagicMock()
        user.name = None
        ...
        oauth2.model.User.check_name_available = MagicMock(return_value=True)

        helper.identify(OAUTH2TOKEN)

        oauth2.toolkit.get_action.assert_called_once_with("organization_member_create")
        membership_action.assert_called_once()
        args, _ = membership_action.call_args
        self.assertEqual("default-org", args[1]["id"])
        self.assertEqual("editor", args[1]["role"])
        self.assertEqual(user.name, args[1]["username"])

    @httpretty.activate
    def test_identify_default_role_falls_back_to_member(self):
        helper = self._helper()
        oauth2.toolkit.config.update(
            {
                "ckan.oauth2.default_organization": "default-org",
        ...
        user_info = {
            self._user_field: "another",
            self._email_field: "another@example.com",
        }
        httpretty.register_uri(
            httpretty.GET,
            self._profile_api_url,
        ...
        args, _ = membership_action.call_args
        self.assertEqual("default-org", args[1]["id"])
        self.assertEqual("member", args[1]["role"])
        self.assertEqual(user.name, args[1]["username"])
    ...
    @patch.dict(
        os.environ, {"OAUTHLIB_INSECURE_TRANSPORT": "", "REQUESTS_CA_BUNDLE": ""}
    )
